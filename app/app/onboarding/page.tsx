...
import { createHouseholdAction, joinHouseholdAction } from "@/app/app/onboarding/actions";
...
export default async function HouseholdOnboardingPage({
  ...
  if (household) {
    return (
      <>
        <PageIntro title="Household ready" description="Your account is connected to a household workspace." />
        <Card>
          <div className="flex items-start gap-3">
            ...
            <div className="min-w-0 flex-1">
              <h1 className="text-lg font-semibold text-slate-950">{household.name}</h1>
              ...
              {params.created ? (
                <p className="mt-3 rounded-lg bg-green-50 p-3 text-sm text-green-700">
                  Household created. Share the code only with people you want to invite.
                </p>
              ) : null}
              <Link
                href="/app"
  ...
  return (
    <>
      <PageIntro title="Set up household" description="Create a new household workspace or join one using a household code." />

      {params.error ? <p className="mb-4 rounded-lg bg-red-50 p-3 text-sm text-red-700">{params.error}</p> : null}

      <div className="grid gap-6 lg:grid-cols-2">
        <Card>
          <CardHeader title="Create household" description="Start a new FamilyLedger workspace." />
          <form action={createHouseholdAction} className="grid gap-4">
            <Field label="Household name">
              <Input name="householdName" maxLength={80} placeholder="Rifqy Family Home" required />
            </Field>
